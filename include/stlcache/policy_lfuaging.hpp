    ...
    template <time_t Age> struct policy_lfuaging {
        template <typename Key, template <typename T> class Allocator>
            struct bind : _policy_lfuaging_type<Age,Key,Allocator> { 
                bind(const bind& x) : _policy_lfu_type<Key,Allocator>(x), _policy_lfuaging_type<Age,Key,Allocator>(x)  { }
                bind(const size_t& size) : _policy_lfu_type<Key,Allocator>(size), _policy_lfuaging_type<Age,Key,Allocator>(size) { }
            };
    };
}
